...
public class InConfirmActivity extends BaseActivity {
    ...
    @BindView(R.id.txt_fs_staff)TextView tFSStaffIN;

    private String TAG = InConfirmActivity.class.getSimpleName();
    private SessionManager sessionManager;
    private String staffId, staff_code, fName, lName, barcode, brand, currentDateTimeString, customer,
            fsStaff;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_in_confirm);
...
//        fsStaff = i.getStringExtra("FSStaff");
    ...
    public void save(View view) {
        if (customer == null || customer.equalsIgnoreCase(null)){
            saveStockInNoCustomer();
        }else{
            saveStockInWithCustomer();
        }
    }

    private void saveStockInNoCustomer(){
        final ProgressDialog dialog = ProgressDialog.show(this, "", "loading...");

        ApiService apiService = RestApi.getClient().create(ApiService.class);

        Call<Inventory> call = apiService.stockInNoCstmr(staff_code, fsStaff, brand, barcode);
        call.enqueue(new Callback<Inventory>() {
            ...
            public void onResponse(Call<Inventory>call, Response<Inventory> response) {
                ...
                if (response.code() == 200 && response.body().getStatus() ) {
                    ...
                    AlertDialog alertDialog = new AlertDialog.Builder(InConfirmActivity.this).create();
    ...
    private void saveStockInWithCustomer(){
        final ProgressDialog dialog = ProgressDialog.show(this, "", "loading...");

        ApiService apiService = RestApi.getClient().create(ApiService.class);

        Call<Inventory> call = apiService.stockInWithCstmr(staff_code, fsStaff, brand, barcode, customer);
        call.enqueue(new Callback<Inventory>() {
            @Override
            public void onResponse(Call<Inventory>call, Response<Inventory> response) {
                dialog.dismiss();
